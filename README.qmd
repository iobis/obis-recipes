...
```{r ggplot-example, fig.width = 7, fig.height = 4.5}
library(obisrecipes)
library(ggplot2)
library(robis)
...
```{r ggplot-example2, fig.width = 7, fig.height = 4.5}
occ_acan <- occurrence(
...
Using H3 grid (first example uses h3j source from the [`mapgl` package](https://walker-data.com/mapgl/reference/add_h3j_source.html), and is not suitable for very large datasets. For larger datasets uses the H3 DeckGL widget, the second and third examples).
