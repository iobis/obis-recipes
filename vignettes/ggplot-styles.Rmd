...
`scale_color_obis_cat()` and `scale_fill_obis_cat()` apply one of three
...
IBM's 5-color palette (blue, purple, magenta, orange, gold) is a good choice
when the number of categories is ≤ 5 and a cooler, more corporate look is
preferred. It works well on both light and dark backgrounds.
...
`scale_fill_obis_cont()` and `scale_color_obis_cont()` offer nine palettes —
five multi-hue sequential, two diverging, and two single-hue sequential:
...
a reference temperature, a species mean, etc.). Both are 11-stop ColorBrewer
palettes with a white neutral centre.
...
`"blues"` and `"greens"` are 9-stop ColorBrewer palettes running from a near-
white tint to a rich dark hue. They work well whenever a single variable needs
to be highlighted without introducing hue contrast — for example, highlighting
species richness or chlorophyll concentration on a neutral basemap.
